...
2. **Product/service catalog** (this seeds the local `categories.md` file, not a Sheets tab — see SPEC §3.4)
   - List your products/services.
   - For each, which types of businesses typically need it? (e.g. "banners → schools, gyms, construction, event venues")
3. **Target geography**
   - Which US ZIP code(s) should discovery search around? (ZIP-code based, not lat/lng + custom radius — SPEC §3.4a. List one ZIP per distinct area; add more ZIPs for broader coverage rather than widening a radius.)
...
5. **Google Cloud credentials**
   ...
   - If no: pause here. Provide the operator with (or point them to) Google's official setup docs for: creating a project, enabling both APIs, creating a service account, generating a JSON key, and sharing the target Google Sheet with the service account's email address (Editor access). **Do not proceed past this step without the key file in hand.** All credentials are supplied via environment variables into the install's `.env` — never pasted into Sheets, Docs, chat, or git (SPEC §12).
6. **AgentMail credentials**
   - AgentMail is the sole supported email provider (locked decision, 2026-09-07). Do you have an AgentMail account and API key?
...
2. Create tabs exactly per `SPEC.md` §3: `Leads`, `History`, `Config`, `Errors`, `Comms_Threads`. (Categories/product-fit catalog lives in the local `categories.md` file, not a Sheets tab — see SPEC §3.4.)
3. Populate header rows per the column definitions in SPEC §3.1–3.6.
4. Populate `Config` tab from questionnaire answers (business profile, geography as ZIP codes, target business types, cadences, guardrails, notification routing, and the Template & Collateral Map from §2.6a).
5. Populate `categories.md` at the repo root from the operator's product/service catalog (questionnaire §1.2).
...
2. For each product category in `categories.md`, create or import at least one email template Doc with the standard placeholder syntax (`{{business_name}}`, `{{product_fit_category}}`, etc.) — do not go live with a category that has zero template.
...
- [ ] Google Sheet created, all 5 tabs present with correct headers (`Leads`, `History`, `Config`, `Errors`, `Comms_Threads`)
- [ ] `categories.md` populated at repo root with at least one product category and no placeholder San Antonio note text left in
...
- [ ] Operator has been shown: where leads live, where drafts live, where comms history lives (`Comms_Threads`), where templates/collateral live (Drive folder), how to change Config values, how to edit `categories.md`, how to mark a lead `lost`/`won` manually if desired
...
- San Antonio ZIP code(s) and top 3–5 target business types — pin down with Rick when Google Cloud key is in hand.
